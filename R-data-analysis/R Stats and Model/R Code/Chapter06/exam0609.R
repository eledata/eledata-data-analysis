toothpaste<-data.frame(
  ...
  X2= c( 5.50,6.75,7.25,5.50,7.00,6.50,6.75,5.25,5.25,6.00,
   ...
   Z = c( 7.38,8.51,9.52,7.50,9.33,8.28,8.75,7.87,7.10,8.00,
         7.89,8.15,9.10,8.86,8.90,8.87,9.26,9.00,8.75,7.95,
         7.65,7.27,8.00,8.50,8.75,9.21,8.27,7.67,7.93,9.26)
)

lm.sol<-lm(Z~X1+X2,data=toothpaste)
summary(lm.sol)

attach(toothpaste)
plot(Z~X1); abline(lm(Z~X1))

lm2.sol<-lm(Z~X2+I(X2^2))
...
result1 <- lm(Y~X1+X2, data = toothpaste)
summary(result1)


plot(Y~X1,data = toothpaste)
abline(lm(Y~X1,data = toothpaste))

plot(Y~X2,data = toothpaste)
abline(lm(Y~X2,data = toothpaste))

attach(toothpaste)
x <- seq(min(X2), max(X2), len = 200)
y <- predict(lm(Y~X2+I(X2^2), data.frame(X2 = x)))
plot(Y~X2,data = toothpaste)
line(x,y)      
